...
@Component({
    templateUrl: './asamblea-general-asociados.component.html',
    styleUrls: ['./asamblea-general-asociados.component.scss'],
    selector: 'app-asamblea-general-asociados',
    changeDetection: ChangeDetectionStrategy.OnPush,
    providers: [UncommentHtmlPipe]
})
export class AsambleaGeneralAsociadosComponent implements OnInit, OnDestroy {
    ...
    /** Lista de programas con sus opciones */
    public programmingList: ASProgram[] = [];
    /** Lista de programas y opciones */
    public allPrograms: ASProgram[] = [];
    ...
    public Presidente: { is: boolean; sign: string | undefined; summoned: Convocado | undefined } = { sign: undefined, is: false, summoned: undefined };

    public Secretario: { is: boolean; sign: string | undefined; summoned: Convocado | undefined } = { sign: undefined, is: false, summoned: undefined };
    ...
    init() {
        ...
        if (this.socketService.socket instanceof Socket) {
            this.socketService.socket.on('addSign-acta-emit', (data: { path: string; rol: string; }) => {
                ...
                this.rol === 'Administrador' && this.notify.showAlert(`El ${data.rol} ha firmado`);
    ...
    private getSummonedList() {
        this.accesoService.getListaConvocados(this.userInfo?.id_reunion as number).subscribe(data => {
            ...
            if (this.allPrograms.length) {
                const elections = this.allPrograms.filter(program => +program.tipo === 5 && ['Presidente', 'Secretario'].includes(program.rol_acta_descripcion as string));
                ...
                const presidente = this.summonedList.find(item => item.id_convocado_reunion === Presidente?.id_convocado_reunion);
                const secretario = this.summonedList.find(item => item.id_convocado_reunion === Secretario?.id_convocado_reunion);
                presidente && (this.Presidente.summoned = presidente);
                secretario && (this.Secretario.summoned = secretario);
            }
            this.cdr.markForCheck();
    ...
    private getSummonedLoggedinList() {
        this.accesoService.getSummonedLoggedinList(this.adminUser.id_reunion as number).subscribe(data => {
            ...
            this.asistencia = this.summonedLoggedinList.length;
            this.participation = this.summonedLoggedinList.reduce((result, item) => result + ((1 / this.summonedList.filter(elm => [0, 2].includes(+elm.tipo)).length) * 100), 0);
            this.cdr.markForCheck();
        });
    }
    ...
    public getProgramming = () => {
        this.accesoService.getProgramacionReunion(this.userInfo?.id_reunion as number, this.summonedUser.id_convocado_reunion).subscribe({
            next: (data) => {
                ...
                this.programmingList = data.response.filter(item => ![3, 4].includes(+item.estado)).map(item => ({
                    ...item, opciones: item.opciones.filter(elm => ![3, 4].includes(+elm.estado)).sort((a, b) => +a.orden - +b.orden),
                })).sort((a, b) => +a.orden - +b.orden);

                this.allPrograms = this.programmingList.reduce((result, item) => {
                    if (![3, 4].includes(+item.estado)) {
                        result.push(item);
                        item.opciones.forEach(opt => ![3, 4].includes(+opt.estado) && result.push(opt))
                    }
                    return result;
                }, [] as ASProgram[]);
